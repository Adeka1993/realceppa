...
class ThreadTestCase(TestCase):
    #Funcion setUp, es la inicialización. Aunque se ejecute un test unitario de una fucnión concreta, 
    # esto se ejecuta y debe obviarse en los test individuales
    def setUp(self):
    ...
    def test_filter_thread_by_users(self):
        ...
        #Esta es la función que comprueba. Comprobar que el hilo 1 se compone de lso dos usaurios
        self.assertEqual(self.thread, threads[0])

    def test_filter_non_existent_thread(self):
        threads = Thread.objects.filter(users=self.user1).filter(users=self.user2)

        #Esta es la función que comprueba. Comprobar que el hilo está vacío porque no añadimos a nadie a la conversación
    ...
    def test_add_messages_to_thread(self):
        ...
        #Esta es la función que comprueba. Comprobar que se añade al hilo dos usaurios y una conversación
